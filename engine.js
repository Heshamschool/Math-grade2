...
const MathEngine = (function(){
  ...
  function detectTopic(title, chapterDefault){
    if(!title) return chapterDefault || 'addition';
    ...
    if(/جدول|تمثيل|بيانات|الإشارات|الأعمدة|بالصور|إمكانية|الحدث/.test(title)) return 'data';
    return chapterDefault || 'addition';
  }
  ...
  const TOPIC_META = {
    ...
    data:        { label:'تمثيل البيانات',       icon:'📊', prompt:'اقرأ الجدول وأجب' },
  };
  ...
  function genDataRead(){
    const pool = [
      {name:'التفاح', icon:'🍎'}, {name:'الموز', icon:'🍌'}, {name:'العنب', icon:'🍇'},
      {name:'البرتقال', icon:'🍊'}, {name:'الفراولة', icon:'🍓'}, {name:'البطيخ', icon:'🍉'}
    ];
    const chosen = shuffle(pool).slice(0,3);
    const used = new Set();
    const counts = chosen.map(()=>{
      let c;
      do{ c = randInt(2,9); }while(used.has(c));
      used.add(c);
      return c;
    });
    ...
    const parts = [];
    chosen.forEach((c,i)=>{
      parts.push({t:'s', v:c.icon});
      parts.push({t:'n', v:counts[i]});
      if(i < chosen.length-1) parts.push({t:'s', v:'   '});
    });

    const qType = randInt(0,2);
    let prompt, answer;
    if(qType === 0){
      const idx = randInt(0, chosen.length-1);
      prompt = 'كم عدد ' + chosen[idx].name + ' في الجدول؟';
      answer = counts[idx];
    } else if(qType === 1){
      prompt = 'ما أكبر عدد في الجدول؟';
      answer = Math.max(...counts);
    } else {
      prompt = 'ما مجموع الأعداد في الجدول؟';
